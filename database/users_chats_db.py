...
from info import DATABASE_URI, DB_AUTO_DELETE, AUTO_FFILTER, AUTO_DELETE, AUTO_DELETE2, DATABASE_NAME, IMDB, IMDB_TEMPLATE, MELCOW_NEW_USERS, P_TTI_SHOW_OFF, SINGLE_BUTTON, SPELL_CHECK_REPLY, PROTECT_CONTENT
import datetime
import pytz
class Database:
    ...
    async def get_notcopy_user(self, user_id):
        ...
        if not user:
            res = {
                "user_id": user_id,
                "last_verified": datetime.datetime(2020, 9, 18, 0, 0, 0, tzinfo=ist_timezone),
                "second_time_verified": datetime.datetime(2019, 9, 18, 0, 0, 0, tzinfo=ist_timezone),
            }
    ...
    async def use_second_shortener(self, user_id):
        ...
        if not user.get("second_time_verified"):
            ist_timezone = pytz.timezone('Asia/Kolkata')
            await self.update_notcopy_user(user_id, {"second_time_verified":datetime.datetime(2019, 9, 18, 0, 0, 0, tzinfo=ist_timezone)})
            user = await self.get_notcopy_user(user_id)
        ...
        if await self.is_user_verified(user_id):
            ...
            if time_difference > datetime.timedelta(seconds=21600):
